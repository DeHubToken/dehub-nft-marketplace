...
abstract contract IExchangeCore {
  ...
  function cancelOrdersUpTo(uint256 targetOrderEpoch) external virtual;
  ...
  ) external payable virtual returns (bool fulfilled);
  ...
  /// @param order Order struct containing order specifications.
  function cancelOrder(LibOrder.Order memory order) external virtual;
  ...
  function getOrderInfo(
    LibOrder.Order memory order
  ) external view virtual returns (LibOrder.OrderInfo memory orderInfo);
}
