...
import {AssetProxyRegistry} from "./AssetProxyRegistry.sol";
  ...
  MarketplaceRegistry
{
  ...
  function _assertFillable(
    ...
    if (orderInfo.orderType == LibOrder.OrderType.LIST) {
      ...
      if (
        erc20TokenAddress == address(0) && msg.value < order.takerAssetAmount
      ) {
    ...
    // Validate signature
    if (
      !_isValidOrderWithHashSignature(order, orderInfo.orderHash, signature)
    ) {
  ...
  function _getOrderInfo(
    LibOrder.Order memory order
  ) internal view returns (LibOrder.OrderInfo memory orderInfo) {
  ...
  function _settle(
  ...
  ) internal returns (uint256 protocolFee, uint256 marketplaceFee) {
    ...
    if (protocolFeeCollector != address(0)) {
      ...
      if (
        orderInfo.orderType == LibOrder.OrderType.SWAP && protocolFixedFee > 0
      ) {
        protocolFee = protocolFixedFee;
        protocolAssetData = abi.encodeWithSelector(
      ...
      if (
        ...
        marketplace.feeCollector != address(0) &&
        marketplace.feeMultiplier > 0 &&
        distributeMarketplaceFees
      ) {
